...
if [ -z "$JFD_JAVA_EXEC" ]; then
	if [ -n "$JFD_JAVA_HOME_VAR" ]; then
		...
		echo "to a valid JRE installation folder (e.g. /home/charly/jre1.8.0_131)."
		...
		echo "(e.g. JFORMDESIGNER_JAVA_HOME=/home/charly/jre1.8.0_131)."
	fi
	echo
...
"$JFD_JAVA_EXEC" -Xmx$JFD_MAX_MEM_SIZE $JFD_VM_PARAMS -jar "$JFD_HOME/lib/JFormDesigner.jar" "$@"
